...
export default function Experienceitem({ icon, title, text }) {
    ...
    return (
        <div className='flex flex-row justfy-center items-center p-2' onMouseEnter={() => { sethovermode(true) }} onMouseLeave={() => { sethovermode(false) }}>
            <div className={`transition ease-in-out duration-700 border-[#ff4800] border-[3px] rounded-full mx-5 p-1 ${hovermode ? 'bg-[#ff4800]' : null}`}>
            ...
            <div className='flex flex-col '>
                ...
                <div className='text-ellipsis max-w-[60%]'>
                    <p>{text}</p>
                </div>
            </div>
        </div>
